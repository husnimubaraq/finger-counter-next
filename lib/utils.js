...
export const drawHands = (hands, ctx, showNames = false, videoSize) => {
    ...
    for (let i = 0; i < hands.length; i++) {
        ...
        for (let y = 0; y < hands[i].keypoints.length; y++) {
            ...
            const cx = parseInt(keypoint.x)
            let cy = parseInt(keypoint.y)

            if(keypoint.name === 'thumb_tip'){
                const thumbTipPosition = motions.find(x => x.name === 'thumb_ip').cy

                cy = ((cy + thumbTipPosition) / 2)
            }

            if(keypoint.name !== "wrist"){
                motions.push({
                    name: keypoint.name, cx, cy
                })
            }
        ...
        const fingersValues = CALCULATED_DISTANCE

        for(let z = 0; z < fingersValues.length; z++){
            const finger = fingersValues[z]

            const downFingerPosY = motions.find(x => x.name === finger[0])?.cy ?? 0

            const upperFingerPosY = motions.find(x => x.name === finger[1])?.cy ?? 0

            const isFingerOpen = downFingerPosY > upperFingerPosY

            counter = isFingerOpen ? counter += 1 : counter
        }
